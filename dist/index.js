...
function isXChainSwapContent(content) {
  const validTypes = typeof content.fromChain === "string" && typeof content.toChain === "string" && typeof content.fromToken === "string" && typeof content.toToken === "string" && typeof content.toAddress === "string" && (typeof content.amount === "string" || typeof content.amount === "number");
  if (validTypes) {
    return true;
  }
  return false;
}
var isValidEvmAddress = (address) => {
  return ethers.isAddress(address);
};
var isValidEvmPrivateKey = (key) => {
  const cleanedKey = key.startsWith("0x") ? key.slice(2) : key;
  return /^[0-9a-fA-F]{64}$/.test(cleanedKey);
};
var squidRouterEnvSchema = z.object({
  ...
  const evmValid = data.SQUID_EVM_ADDRESS && data.SQUID_EVM_PRIVATE_KEY && isValidEvmAddress(data.SQUID_EVM_ADDRESS) && isValidEvmPrivateKey(data.SQUID_EVM_PRIVATE_KEY);
  return evmValid;
}, {
...
async function validateSquidRouterConfig(runtime) {
  try {
    const config = {
...
// src/types/index.ts
var nativeTokenConstant = "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE";

// src/providers/squidRouter.ts
var getSDK = (baseUrl, integratorId) => {
...
var SquidRouterProvider = class {
  constructor(squidSDKUrl, squidIntegratorID) {
    this.squidSDKUrl = squidSDKUrl;
    this.squidIntegratorID = squidIntegratorID;
  ...
  getToken(targetChain, targetTokenSymbol) {
  ...
  async getEVMSignerForChain(chain, runtime) {
    try {
      if (chain.chainType === ChainType.EVM) {
        const provider = new ethers2.JsonRpcProvider(chain.rpc);
        return new ethers2.Wallet(runtime.getSetting("SQUID_EVM_PRIVATE_KEY"), provider);
      }
      throw new Error("Cannot instantiate EVM signer for non-EVM chain");
    } catch (error) {
      throw new Error(`Cannot instantiate EVM signer: ${error}`);
    }
  }
...
var initSquidRouterProvider = (runtime) => {
...
var squidRouterProvider = {
  get: async (runtime, _message, _state) => {
    try {
      initSquidRouterProvider(runtime);
...
var xChainSwapTemplate = `Given the recent messages and wallet information below:
...
var approveSpending = async (transactionRequestTarget, fromToken, fromAmount, signer) => {
  const erc20Abi = [
    "function approve(address spender, uint256 amount) public returns (bool)"
  ];
  const tokenContract = new ethers3.Contract(fromToken, erc20Abi, signer);
  try {
...
var xChainSwapAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      if (!(fromTokenObject == null ? void 0 : fromTokenObject.enabled)) {
        throw new Error(
...
var index_default = squidRouterPlugin;
export {
  SquidRouterProvider,
  index_default as default,
  initSquidRouterProvider,
  nativeTokenConstant,
  squidRouterPlugin,
  squidRouterProvider,
  xChainSwapAction,
